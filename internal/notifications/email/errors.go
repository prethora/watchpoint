...
// template resolution with soft-fail fallback, data preparation with timezone
// localization, and delivery via an external EmailProvider (e.g., SendGrid).
//
...
// the AppError code ErrCodeEmailBlocked (returned by SendGridClient).
func IsBlocklistError(err error) bool {
	if errors.Is(err, ErrRecipientBlocked) {
		return true
	}
	// Also check for the AppError code returned by the SendGrid client,
	// which maps 403 Forbidden to ErrCodeEmailBlocked.
